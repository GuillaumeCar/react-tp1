...
- [F.3. Le fichier `package.json`](#f3-le-fichier-packagejson)
- [F.4. Créer un script de build personnalisé](#f4-créer-un-script-de-build-personnalisé)
...
## F.4. Créer un script de build personnalisé
...
6. **Ajoutez un nouveau script nommé `"watch"`** qui permettra de lancer la commande :
	...
	> _**NB :** Inspirez-vous de la commande que l'on a tapée pour le "build" : comme on est dans un script npm, il y a moyen de simplifier la commande ! Si vous ne voyez pas où je veux en venir, relisez donc le **NB** du point F.4.4. ..._
...
Maintenant que votre code compile, vous pouvez passer à la dernière étape : [G. Le destructuring](./G-destructuring.md)